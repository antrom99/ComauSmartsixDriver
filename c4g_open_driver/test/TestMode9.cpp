...
int32_t main(int32_t argc, char *argv[])
{
    ...
    cout << "\n       2. Set mode 9 via PDL2 (ONLY for axis 1)\n"; cout.flush();
    ...
    if (c4gOpen.start())
    {
        ...
        float amplitude = c4gOpen.getCurrentLimit(ARM, AXIS) * CURRENT_COEFF;
        ...
        while (keepGoingOn)
        {
            if (c4gOpen.receive())
            {
                ...
                {
                    if (mode == C4G_OPEN_DRIVING_ON || mode == C4G_OPEN_MODE_0 || mode == C4G_OPEN_MODE_9)
                    {
                        if (mode == C4G_OPEN_MODE_9)
                        {
                            if (c4gOpen.isInDriveOn(ARM))
                            {
                                double omegat = 2 * PI * frequencyHz * ((double)counter * sampleTime);
                                deltaCurrent = (float)(amplitude * sin(omegat));
                                counter++;
                            }

                            c4gOpen.setDeltaCurrent(ARM, AXIS, deltaCurrent);
                        }
