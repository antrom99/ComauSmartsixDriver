...
int32_t main(int32_t argc, char *argv[])
{
    ...
    double txRate[6];
    double calibration_constants[6];
    ...
    for (int32_t i = 0; i < 6; i++) sinAxisEnabled[i] = false;
    ...
    if (c4gOpen.start())
    {
        ...
        for (int32_t i = 0; i < 6; i++)
        {
            ...
            {
                if (i < 4)
                    amplitudeGearRotations[i] = txRate[i] * amplitudeDeg * M_PI / 180 / (2 * M_PI) + calibration_constants[i];
                else if (i == 4)
                    amplitudeGearRotations[i] = txRate[i] * (amplitudeDeg * M_PI / 180 - k54 * ((amplitudeGearRotations[i - 1] - calibration_constants[i - 1]) * 2 * M_PI / txRate[i - 1])) / (2 * M_PI) + calibration_constants[i];
                else if (i == 5)
                    amplitudeGearRotations[i] = txRate[i] * (amplitudeDeg * M_PI / 180 - k65 * ((amplitudeGearRotations[i - 1] + txRate[i - 1] * k54 * ((amplitudeGearRotations[i - 2] - calibration_constants[i - 2]) * 2 * M_PI / txRate[i - 2]) / (2 * M_PI) - calibration_constants[i - 1]) * 2 * M_PI / txRate[i - 1]) + (k65 * k54 - k64) * ((amplitudeGearRotations[i - 2] - calibration_constants[i - 2]) * 2 * M_PI / txRate[i - 2])) / (2 * M_PI) + calibration_constants[i];
            }
        }
